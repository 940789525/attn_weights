...
class Attention(nn.Module):
    def __init__(self, embed_dim, num_heads, lora_dim):
        ...
        self.in_proj_weight = nn.Parameter(torch.empty(3 * embed_dim, embed_dim))  # 权重  QKV3个矩阵合并到了一起
        self.in_proj_bias = nn.Parameter(torch.empty(3 * embed_dim))  # 偏置
        self.out_proj = nn.Linear(embed_dim, embed_dim, bias=True)   # 输出层投影
        ...
        # self.TVPt_LoRA_b @ self.TVPt_LoRA_a-> (3 * embed_dim, embed_dim)  和 上面QKV合并矩阵形状相同
...
class ResidualAttentionBlock(nn.Module):
    def __init__(self, d_model: int, n_head: int, lora_dim: int, frame_num: int, attn_mask=None):
        ...
        if frame_num > 0:
            self.TVPt_Video_Positional_embedding = nn.Parameter(torch.zeros(1, frame_num, 1, d_model))  # 在进行跨片段合并（Cross-clip Merging）之前，为每个即将被合并的片段（clip）添加一个独特的位置标识，从而让注意力机制能够理解和区分它们在时间序列中的位置关系
    ...
    def __init__(self, width: int, layers: int, heads: int, lora_dim: int, merge_layer: List[int], frame_pos: int, frame_num_list: List[int], attn_mask=None):
        ...
        if frame_pos == 0:
            ...
            for _l in range(layers):
                if _l in merge_layer:  # 对需要进行帧合并的层进行特殊处理
...
class VisualTransformer(nn.Module):
    def __init__(self, input_resolution: int, patch_size: int, width: int, layers: int, heads: int, output_dim: int, lora_dim: int, merge_layer: List[int], frame_pos: int, frame_num_list: List[int]):
        ...
        for param in self.conv1.parameters():  # 冻结conv1这个至关重要的图像“Token化”层，以稳定训练并充分利用预训练模型的强大能力
...
class CLIP(nn.Module):
    def __init__(self,
        ...
        self.token_embedding = nn.Embedding(vocab_size, transformer_width)  # 创建一个词嵌入层，它的核心作用是将输入的、代表文本的数字ID序列，转换成一个密集、低维、可学习的实数向量序列（即词向量），为后续的文本编码器提供最初的语义特征输入
