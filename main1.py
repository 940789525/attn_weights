...
def train_epoch(epoch, args, model, train_dataloader, device, n_gpu, optimizer,
    ...
    for step, batch in enumerate(train_dataloader, start=1):
        ...
        max_len_for_weighting = 18.0 # 句子长度超过此值后，权重不再衰减
        ...
        # d. 创建一个动态权重：句子越短，权重越高
        # 权重从1线性衰减到0.1，长度超过max_len_for_weighting后权重保持在0.1
        weight_tensor = 1.0 - 0.9 * torch.clamp(sentence_lengths / max_len_for_weighting, 0, 1)
        
        # e. 计算加权后的沉降分数损失 (直接最小化 sink_score)
        # 我们希望 sink_score 越小越好，所以直接把它作为损失的一部分
        sink_score_loss = (sink_scores_tensor * weight_tensor).mean()
        ...
        meters.update(time=(time.time() - end), data=data_time, retrieval_loss=float(reduced_retrieval_l), sink_loss=float(reduced_sink_l))
        end = time.time()
...
def eval_epoch(args, model, test_dataloader, device, tokenizer):
    ...
    #      # --- 新增: 识别并可视化“反常”高性能短句 ---
    #     target_indices = df[(df['sentence_index']==319)|(df['sentence_index']==14)].index.tolist()
    ...
    #                     save_path = os.path.join(args.output_dir, "attention_maps_anomalies_r2")
